//
//  Progress ViewController.m
//  To-Do List
//
//  Created by Macos on 24/04/2025.
//

#import "Progress ViewController.h"
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    switch (self.priority.selectedSegmentIndex) {
        ...
        case 1:
            for (NSDictionary *task in self.arr ) {
                if ([task[@"priority"] intValue] == 0 && [task[@"condition"] intValue] == 1) {
                    [self.filteredList addObject:task];
                }
            }
        ...
        case 2:
            for (NSDictionary *task in self.arr) {
                if ([task[@"priority"] intValue] == 1 && [task[@"condition"] intValue] == 1) {
                    [self.filteredList addObject:task];
                }
            }
            NSLog(@"%@", self.filteredList);
        ...
        case 3:
            for (NSDictionary *task in self.arr) {
                if ([task[@"priority"] intValue] == 2 && [task[@"condition"] intValue] == 1) {
                    [self.filteredList addObject:task];
                }
            }
            NSLog(@"%@", self.filteredList);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([dataSource[indexPath.row][@"condition"] intValue] == 1 ) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDictionary *selectedTask = dataSource[indexPath.row];
    NSInteger originalIndex = [self.arr indexOfObject:selectedTask];
    
    evc.dict = selectedTask;
    evc.hidesBottomBarWhenPushed = YES;
    evc.index = originalIndex;
